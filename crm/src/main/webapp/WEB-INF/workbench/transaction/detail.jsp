...
<body id="tBody">
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 10px;">
			...
			<div style="width: 300px;position: relative; left: 200px; top: -20px;"><b id="name">动力节点-交易01</b></div>
...
<script>
	$(function () {
		refreshTranDetail();
		refreshStagePossibility();
	})


	//展示数据
	function refreshTranDetail() {
		$.ajax({
			url: "/crm/workbench/transaction/queryTransactionDetail/"+'${id}',
			type: "get",
			dataType: "json",
			success: function (result) {
				...
				$("#expectedDate").text(result.expectedDate);
				$("#customerId").text(result.customerId);
				$("#stage").text(result.stage);
				$("#type").text(result.type);
				$("#possibility").text(result.possibility);
				$("#source").text(result.source);
				$("#activityId").text(result.activityId);
				$("#contactsId").text(result.contactsId);
				$("#createBy").text(result.createBy);
				$("#createTime").text(result.createTime);
				$("#editBy").text(result.editBy);
				$("#editTime").text(result.editTime);
				$("#description").text(result.description);
				$("#contactSummary").text(result.contactSummary);
				$("#nextContactTime").text(result.nextContactTime);

				TranRemark(result)
				TranHistory(result.tranHistory)
			}
		})
	};
	//展示阶段状态-发请求
	function refreshStagePossibility() {
		$.ajax({
		    url: "/crm/workbench/transaction/queryStages",
		    type: "get",
		    data: {'id':'${id}'},
		    dataType: "json",
		    success: function (result) {
		        console.log(result)
				refreshStage(result.list)
		    }
		});
	...
	//交易备注
	function TranRemark(data) {
		$.each(data.tranRemark,function (index,tranRemark) {

			$("#remarkDiv").before("<div class=\"remarkDiv\" style=\"height: 60px;\">\n" +
					"\t\t\t<img title=\"zhangsan\" src=\"/crm/image/user-thumbnail.png\" style=\"width: 30px; height:30px;\">\n" +
					"\t\t\t<div style=\"position: relative; top: -40px; left: 40px;\" >\n" +
					"\t\t\t\t<h5>"+tranRemark.noteContent+"</h5>\n" +
					"\t\t\t\t<font color=\"gray\">交易</font> <font color=\"gray\">-</font> <b>"+data.customerId+"-"+data.name+"</b> <small style=\"color: gray;\"> "+tranRemark.createTime+" 由 "+tranRemark.createBy+"</small>\n" +
					"\t\t\t\t<div style=\"position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;\">\n" +
					"\t\t\t\t\t<a class=\"myHref\" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-edit\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>\n" +
					"\t\t\t\t\t&nbsp;&nbsp;&nbsp;&nbsp;\n" +
					"\t\t\t\t\t<a class=\"myHref\" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-remove\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>\n" +
	...
	$("#stageDiv").on("click",".mystage",function () {
		$.ajax({
		    ...
		    success: function (result) {
				...
				refreshStage(result.list)
				//刷新交易历史
				let tranHistory = result.tranHistory
				$("#tranHistoryTbody").append("<tr>\n" +
	...
	//阶段历史
	function TranHistory(data){
		$("#tranHistoryTbody").html("")
	...
	function refreshStage(data){
		...
		for (let i = 0; i < data.length; i++) {
			let stageEle = data[i]
			if(stageEle.type == "绿圈"){
				$("#stageDiv").append("<span class=\"glyphicon glyphicon-ok-circle mystage\" data-toggle=\"popover\" " +
						"data-placement=\"bottom\" index="+stageEle.index+" data-content="+stageEle.title+" style=\"color: #90F790;\"></span>")
			}
			if(stageEle.type == "锚点"){
				$("#stageDiv").append("<span class=\"glyphicon glyphicon-map-marker mystage\" data-toggle=\"popover\" " +
						"data-placement=\"bottom\" index="+stageEle.index+" data-content="+stageEle.title+" style=\"color: #90F790;\"></span>")
			}
			if(stageEle.type == "黑圈"){
				$("#stageDiv").append("<span class=\"glyphicon glyphicon-record mystage\" " +
						"data-toggle=\"popover\" index="+stageEle.index+" data-placement=\"bottom\" data-content="+stageEle.title+"></span>")
			}
			if(stageEle.type == "黑x"){
			...
			if(stageEle.type == "红x"){
				$("#stageDiv").append("<span style='color: red' class=\"glyphicon glyphicon-remove mystage\" " +
						"data-toggle=\"popover\" index="+stageEle.index+" data-placement=\"bottom\" data-content="+stageEle.title+"></span>")
			}
			$("#stageDiv").append("-----------");
		}
		$("#stageDiv").append(new Date().toLocaleTimeString())
		refreshCSS()
	}
